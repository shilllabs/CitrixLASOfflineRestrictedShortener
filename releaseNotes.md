...
- Added auto-detection of the Citrix Licensing Server install location.
- The tool now locates `ctxLasOfflineActivationTool.exe` automatically using two strategies: (1) inspecting the Windows registry for Citrix Licensing install paths, and (2) scanning every available drive letter for the standard install path. This means installations on a secondary drive (D:, E:, etc.) are found without manual configuration.
- **Step 1, Option A** now displays the detected install location in a read-only box for confirmation, with two controls:
  - **Re-detect** — re-runs auto-detection.
  - **Browse...** — manually select the Citrix Licensing Server `LS` folder to override the detected path.
- The display shows whether the activation tool was actually found at the detected location (green) or not (orange warning).
- **Step 3 (Activate)** uses the same detected location for the import command.
- The CLI `generate` command also auto-detects the install location and prints it.

---

## v3.1.0 — 2026-06-07

### Public Key Display Fix

- Fixed the Public Key display box on the "Step 2: Decode" tab.
- It now shows the raw public key text exactly as stored, preserving the literal `\n` characters, instead of converting them into actual line breaks.
